...
boardNav.innerHTML = navItems
  .map(
    ({ category, color }) => `
  <button onclick="showNews('${category}')" class="nav--item ${
      category === "all" && "active"
...
const latestNews = data.sort(({ date: dateA }, { date: dateB }) =>
  dateA > dateB ? -1 : 1
);
...
function showNews(selectedCategory) {
  ...
  if (selectedCategory === "all") {
    ...
    const specificNews = data.filter(
